...
int main()
{
		...
		target->verdict = -NF_DROP - 1;

			unsigned char* mask = (unsigned char*) malloc(targetOffset);
			memset(mask, 0xFF, targetOffset);
		
        int x = iptc_delete_entry("INPUT", e, mask, h);
        if (!x)
        {
                printf("iptc_delete_entry::Error delete entry: %s\n", iptc_strerror(errno));
                result = -1;
                goto end;
        }
